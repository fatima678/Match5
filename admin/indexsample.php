<?php 
  ...
  $sql = "SELECT `name`, `gender`,`profile_img` FROM `candidate` WHERE 1";
...
<?php  
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
          $name = $row['name'];
            echo '<div class="card">';
                echo '<img src="' . $row['profile_img'] .'" alt="Avatar" style="width:100%">';
                echo '<div class="container">';
                  echo '<h4>' . $row['name'] .'</h4>'; 
                  echo '<p>Status</p>';
                  echo '<p>City </p>';
                echo '</div>';
                echo '<button class="btn ">View Profile</button>';
            echo '</div>';
        }//while  
    }//if
?>
